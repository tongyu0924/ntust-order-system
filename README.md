...
│[Tab]├── orderController.go  # 點餐相關的業務邏輯

│[Tab]└── userController.go   # 用戶相關的業務邏輯

│
├── /models                # 模型：資料結構和 Firebase 交互定義

│[Tab]├── order.go           # 點餐的數據模型
...
│[Tab]└── router.go          # 所有路由的統一管理

│
├── /services              # 服務：處理具體功能邏輯，如 Firebase 操作

│[Tab]├── firebaseService.go  # Firebase 的初始化和操作

│[Tab]└── orderService.go     # 點餐邏輯（如創建訂單、查詢訂單）

│

├── /utils                 # 公用工具：放置一些工具函數或通用邏輯

│[Tab]└── response.go        # 標準化的 API 回應結構

│

├── /config                # 配置：存放專案相關的配置文件

│[Tab]└── firebaseConfig.go  # Firebase 的配置初始化

│

├── /views                 # 前端視圖：HTML 或其他模板文件（如果需要網頁前端）

│[Tab]└── index.html         # 首頁或其他視圖
